...
public class AdbConnection implements Closeable {
						...
						case AdbProtocol.CMD_CNXN:
							synchronized (conn) {
                                ...
                                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                                    conn.features = parseConnectionString(new String(msg.getPayload(), StandardCharsets.UTF_8));
                                }
	 ...
	 * @throws InterruptedException If we are unable to wait for the connection to finish
	 */
	public synchronized AdbStream open(String destination) throws UnsupportedEncodingException, IOException, InterruptedException
	{
		...
		if (stream.isClosed())
			throw new ConnectException("Stream open actively rejected by remote peer for destination=".concat(destination));
	...
	private String[] parseConnectionString(String connectionString) {
		return connectionString.split(";features=")[1].split(",");
	}
}
